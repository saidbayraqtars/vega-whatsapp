...
//
// ÇOK HESAP (v1.5): bu dosya artık TEK bir oturum değil, oturum FABRİKASIDIR.
// createSession({ id, authDir, label }) bağımsız bir Baileys oturumu üretir —
// kendi auth klasörü, kendi QR'ı, kendi durumu, kendi sayacı. Böylece 2/3/4
// telefon AYNI ANDA QR okutabilir. Hangi hesabın hangi mesajı göndereceğine
// accounts.js karar verir (sticky eşleme + sırayla dağıtım).
// Eski tekil API (initializeWhatsApp, sendMessage…) accounts.js üzerinden
// sağlandığı için burada MODÜL SEVİYESİ durum kalmadı.
...
// ─── Bağlantı olay günlüğü (kalıcı, TÜM hesaplar ortak dosya) ────────────────
...
// eklenir; dosya 200KB'ı aşınca son yarısı tutulur. Çok hesapta satır başına
// hesap kimliği yazılır (hangi numara koptu, ayırt edilebilsin).
const waEventRaw = (line) => {
    try {
        ensureDir(DATA_DIR);
...
// ─── WA protokol sürümü önbelleği (ortak) ────────────────────────────────────
...
// Baileys ESM modülü tüm oturumlarda ORTAK yüklenir (tek import).
let _baileys = null;
...
// ═══════════════════════════════════════════════════════════════════════════
//  OTURUM FABRİKASI — her çağrı bağımsız bir WhatsApp hesabı yönetir
// ═══════════════════════════════════════════════════════════════════════════
//  opts: { id, authDir, label }
//    id      → hesap kimliği ('main', 'a2'…). Günlük/sayaç dosyası adında geçer.
//    authDir → Baileys çok-dosyalı oturum klasörü (hesap başına AYRI olmalı).
//    label   → kullanıcıya gösterilen ad ("Satış hattı" gibi).
function createSession({ id, authDir, label = '' }) {
    const AUTH_DIR = authDir;
    const STATS_PATH = path.join(DATA_DIR, id === 'main' ? 'wa-stats.json' : `wa-stats-${id}.json`);
    ...
    let qrTimer = null;
    let meId = null;
    // Hesap kaldırıldı mı? Kaldırma sırasında sock.logout() bir 'close' olayı
    // üretiyor, o da 401'i "gerçek logout" sayıp scheduleInit ile YENİDEN bağlanma
    // kuruyordu → silinen numara hayalet oturum olarak QR döngüsüne giriyordu.
    // Bu bayrak açıkken hiçbir yeniden başlatma yolu çalışmaz.
    let disposed = false;

    // Gelen mesaj işleyici (AI oto-yanıt botu için). accounts.js kaydeder; sock her
    // yeniden bağlanışta baştan kurulduğu için dinleyici initialize içinde bağlanır
    // ve bu callback'i çağırır (removeAllListeners sonrası kaybolmasın diye).
    ...
    // kalsın diye doğrudan antiban require etmez — accounts.js bağlar.
    ...
    const waEvent = (line) => waEventRaw(`[${id}] ${line}`);
    ...
    const cacheSentMessage = (mid, message) => {
        if (!mid || !message) return;
        sentMsgCache.set(mid, message);
        if (sentMsgCache.size > 400) sentMsgCache.delete(sentMsgCache.keys().next().value);
    };

    // ─── Günlük gönderim sayacı (kalıcı, HESAP BAŞINA) ───────────────────────
    ...
    const scheduleInit = (ms) => {
        ...
        retryTimer = setTimeout(() => {
            retryTimer = null;
            if (!isReady && !isInitializing) initialize();
        }, ms);
    };
    ...
    const wipeAuth = () => {
        if (fs.existsSync(AUTH_DIR)) {
            ...
            catch (err) { console.error(`[WhatsApp:${id}] Auth temizlenemedi:`, err.message); }
        }
    };

    const initialize = async () => {
        if (disposed || isReady || isInitializing) return getStatus();

        isInitializing = true;
        lastError = null;
        currentQR = null;
        clearQrTimer();
        console.log(`[WhatsApp:${id}] Başlatılıyor...`);
        ...
        try {
            ...
            try {
                ...
                console.log(`[WhatsApp:${id}] WA protokol sürümü:`, version.join('.'));
            } catch (err) {
                version = loadCachedVersion() || undefined;
                console.warn(`[WhatsApp:${id}] Sürüm sorgulanamadı,`, version ? `önbellek: ${version.join('.')}` : 'varsayılan kullanılacak.', '—', err.message);
            }
            ...
            sock.ev.on('messages.upsert', async ({ messages, type }) => {
                ...
                for (const m of (messages || [])) {
                    try {
                        ...
                        // accountId: yanıt AYNI hattan gitsin (müşteri iki numaradan mesaj almasın).
                        await incomingHandler({ phone, text, jid, id: m.key?.id || null, accountId: id });
                    } catch (err) {
            ...
            sock.ev.on('connection.update', (update) => {
                ...
                if (qr) {
                    ...
                    clearQrTimer();
                    console.log(`[WhatsApp:${id}] QR hazır — telefonla okutun.`);
                }
                ...
                if (connection === 'open') {
                    ...
                    qrCycles = 0;
                    clearQrTimer();
                    console.log(`[WhatsApp:${id}] Bağlantı kuruldu!`, meId || '');
                    waEvent(`open ${meId || ''}`);
                ...
                if (connection === 'close') {
                    ...
                    console.log(`[WhatsApp:${id}] Bağlantı kapandı. Kod: ${statusCode}, mesaj: ${errMsg}`);
                    ...
                    if (shouldWipe) {
                        ...
                        // "Yenile"ye basınca (refresh) sayaçlar sıfırlanıp devam eder.
                        isInitializing = false;
                        clearRetry();
                        if (!sessionRevoked) {
            ...
            qrTimer = setTimeout(() => {
                if (!isReady && !currentQR) {
                    console.warn(`[WhatsApp:${id}] QR zaman aşımı, yeniden deneniyor.`);
            ...
            cleanupSocket();
            clearQrTimer();
            console.error(`[WhatsApp:${id}] Başlatma hatası:`, err.message);
            scheduleInit(5000);
        }
    ...
    const refresh = async () => {
        console.log(`[WhatsApp:${id}] Manuel yenileme (oturum sıfırlanır).`);
    ...
    // Çıkış yap (auth silinir → yeni QR ister). shutdown=true ise yeniden
    // başlatılmaz; hesap silinirken kullanılır.
    const logout = async ({ shutdown = false } = {}) => {
        // Kaldırma: bayrağı ÖNCE koy — logout'un tetiklediği 'close' olayı
        // yeniden bağlanma zamanlamasın (hayalet oturum).
        if (shutdown) disposed = true;
        clearRetry();
        clearQrTimer();
        ...
        lastConflictAt = 0;
        wipeAuth();
        if (shutdown) { currentQR = null; lastError = 'Hesap kapatıldı.'; }
        return getStatus();
    };
    ...
    const sendMessage = async (phone, text, media = null, opts = {}) => {
        ...
        try {
            ...
            return { success: true, id: sent?.key?.id || null, accountId: id };
        } catch (err) {
            console.error(`[WhatsApp:${id}] Gönderim hatası →`, clean, err.message);
            waEvent(`send-fail to=${clean} err=${err.message}`);
            try { stats.recordFail(opts.channel); } catch { /* yok say */ }
            return { success: false, error: err.message, accountId: id };
        }
    };
    ...
    const deleteMessage = async (phone, msgId) => {
        if (!isReady || !sock) return { success: false, error: 'WhatsApp bağlı değil.' };
        if (!msgId) return { success: false, error: 'Mesaj kimliği yok.' };
        const clean = normalizePhone(phone);
        if (!clean) return { success: false, error: 'Geçersiz numara.' };
        const jid = `${clean}@s.whatsapp.net`;
        try {
            await sock.sendMessage(jid, { delete: { remoteJid: jid, fromMe: true, id: msgId } });
            waEvent(`recall-ok to=${clean} id=${msgId}`);
            return { success: true };
        } catch (err) {
            waEvent(`recall-fail to=${clean} id=${msgId} err=${err.message}`);
    ...
    return {
        ...
        getDailySent,
        waitForReady,
        setIncomingHandler: (fn) => { incomingHandler = fn; },
        setDisconnectHandler: (fn) => { disconnectHandler = fn; },
        setOpenHandler: (fn) => { openHandler = fn; },
        setRevokedHandler: (fn) => { revokedHandler = fn; },
        get client() { return sock; },
    };
}

module.exports = { createSession, toJid, baseDir, DATA_DIR };
